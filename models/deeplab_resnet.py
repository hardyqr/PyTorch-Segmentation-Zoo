...
# modified from: https://raw.githubusercontent.com/speedinghzl/Pytorch-Deeplab/master/deeplab/model.py
...
if __name__ == "__main__":
    model = Res_Deeplab(num_classes=38).cuda()
    x = Variable(torch.FloatTensor(np.random.random((1,3,240,320)))).cuda()
    s = x.data.cpu().numpy().shape
    interp = nn.Upsample(size=(s[2],s[3]), mode='bilinear')
    tmp = model(x)
    print (tmp.size())
    out = interp(tmp)
    print (out.size())
